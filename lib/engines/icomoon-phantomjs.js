  ...
  'export': function (exportOptions, callback) {
    ...
    async.waterfall([
      ...
      function execIcoMoon (cb) {
        ...
        // DEV: PhantomJS allows for sslv3 (broken), sslv2 (broken), and tlsv1 (working)
        // DEV: Fix via https://github.com/twolfson/fontsmith/pull/9
        var phantomjsOptions = exportOptions.phantomjsOptions || ['--ssl-protocol=tlsv1'],
            cmdArr = ['phantomjs'].concat(phantomjsOptions).concat([icomoonPath, tmpPath]);
        exec(quote(cmdArr), cb);
      },
